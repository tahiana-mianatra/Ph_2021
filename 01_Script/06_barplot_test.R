...
univariable_normal_barplots <- function(
    data, 
    metadata, 
    output_dir = here::here("04_Graphic_output", "Univariable_barplot"),
    file_prefix = "barplot"
) {
  ...
  walk(common_vars, ~ {
    tryCatch({
      ...
      # Generate plot - REMOVED coord_flip() and updated aesthetics
      p <- ggplot(var_percentages, aes(x = reorder(!!sym(.x), -percentage),  # Added negative for descending order
                                       y = percentage)) +
        geom_bar(stat = "identity", fill = "steelblue") +
        # coord_flip() REMOVED - now creating vertical bars
        geom_text(
          data = var_percentages %>% 
            mutate(
              text_color = ifelse(percentage < 5, "red", "black"),  # Changed to black for better visibility
              text_vjust = ifelse(percentage < 5, -0.5, 1.5),      # Adjusted vjust logic
              text_size = ifelse(percentage < 5, 4.5, 4)
            ),
          aes(
            label = sprintf("%.1f%%", percentage),
            color = text_color,
            vjust = text_vjust        # Using vjust for vertical positioning
          ),
          size = 4,
          show.legend = FALSE
        ) +
        ...
        scale_y_continuous(limits = c(0, 100),
                           breaks = NULL,
                           expand = expansion(mult = c(0, 0.1))) +  # Added space for labels
        labs(
          x = "",
          y = "",
        ...
        theme(
          ...
          axis.text.x = element_text(face = "bold", colour = "black", hjust = 1),  # Show x labels, angled
          axis.text.y = element_blank(),  # Hide y axis text instead
  ...
  message("Created ", length(common_vars), " bar plots in ", output_dir)
}
#Step 1: select all the relevant variable and use label

barplot_df <- survey %>%
  select(QUEST, intersect(names(survey), barplot_var$variable))
...
common_vars <- intersect(names(barplot_df), names(survey_labels))
complete_coded <- barplot_df
...
#Step 2 Make the graphics direct out of survey result
univariable_normal_barplots(complete_coded, barplot_var)
